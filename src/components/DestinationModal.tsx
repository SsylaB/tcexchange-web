...
function DestinationModal({ destination, onClose }: Props) {
    const {
        university_name, country, location,
        exchange_type, languages, description, url, short_name
    } = destination;
        ...
        <div className="modal-backdrop" onClick={handleBackdropClick}>
            <div className="modal-panel" role="dialog" aria-modal="true">
                ...
                <div className="modal-header">
                    ...
                    <div>
                        {short_name && short_name !== university_name.split(" ")[0] && (
                            <span className="modal-shortname">{short_name}</span>
                        )}
                ...
                <div className="modal-badges">
                    {exchange_type && (
                        <span className={`modal-badge modal-badge--${exchange_type === "Erasmus" ? "erasmus" : "bilateral"}`}>
                            {exchangeEmoji} {exchange_type}
                        </span>
                    )}
                    {langArray.map((lang) => (
                        <span key={lang} className="modal-badge modal-badge--lang">🗣️ {lang}</span>
                    ))}
                </div>

                {url && (
                    <a href={url} target="_blank" rel="noopener noreferrer" className="modal-cta">
