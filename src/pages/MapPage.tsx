...
function MapPage() {
    const [destinations, setDestinations] = useState<DestinationRaw[]>([]);
    const [loading, setLoading] = useState(true);
    const [selectedDest, setSelectedDest] = useState<DestinationRaw | null>(null);
    const [selectedCoords, setSelectedCoords] = useState<LatLngTuple | null>(
        null,
    );
    ...
    function handleSelect(dest: DestinationRaw, coords: LatLngTuple) {
    ...
    const flag = selectedDest
        ? (COUNTRY_FLAG[selectedDest.country] ?? "🌍")
        : null;
    const langArray =
        selectedDest?.languages?.split(",").map((l) => l.trim()) ?? [];
    ...
    return (
        <div className="map-page">
            <div className="map-page__header">
                <h1 className="page-title page-title--standard page-title--centered">
                    Carte des Échanges
                </h1>
                <p style={{ color: "var(--text-soft)" }}>
                    Explorez les opportunités autour de vous
                </p>
            </div>

            <div
                className={`map-split ${selectedDest ? "map-split--open" : ""}`}
            >
                    ...
                    <div className="map-split__panel">
                        ...
                        <div className="map-split__panel-inner">
                            ...
                            {selectedDest.short_name && (
                                <span className="panel__shortname">
                                    {selectedDest.short_name}
                                </span>
                            )}
                            <h2 className="panel__title">
                                {selectedDest.university_name}
                            </h2>
                            ...
                            <div className="panel__badges">
                                {selectedDest.exchange_type && (
                                    <span
                                        className={`badge badge--${selectedDest.exchange_type === "Erasmus" ? "erasmus" : "bilateral"}`}
                                    >
                                        {exchangeEmoji}{" "}
                                        {selectedDest.exchange_type}
                                    </span>
