...
function CatalogPage() {
    ...
    const exchangeTypes = useMemo(
        () =>
            [
                ...new Set(
                    destinations.map((d) => d.exchange_type).filter(Boolean),
                ),
            ] as string[],
        [destinations],
    );
    ...
    const filteredDestinations = useMemo(() => {
        return destinations.filter((d) => {
            ...
            const matchesLanguage =
                selectedLanguage === "" ||
                (d.languages?.split(",").map((l) => l.trim()) ?? []).includes(
                    selectedLanguage,
                );
            const matchesExchangeType =
                selectedExchangeType === "" ||
                d.exchange_type === selectedExchangeType;
    ...
    return (
        <main className="catalog-page">
            ...
            <p className="catalog-page__count">
                ...
                {hasActiveFilters && (
                    <span className="catalog-page__count--active">
                        {" "}
                        · filtres actifs
                    </span>
                )}
            ...
            {visibleCount < filteredDestinations.length && (
                <div className="catalog-load-more-actions">
                    ...
                    >
                        Afficher plus (
                        {filteredDestinations.length - visibleCount} restantes)
                    </button>
                    <button
