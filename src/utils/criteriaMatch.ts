...
const LANGUAGE_CRITERIA = new Set([
    "Anglais", "Espagnol", "Allemand", "Italien", "Russe", "Japonais", "Coréen", "Mandarin/Chinois", "Arabe"
]);
...
function matchesLanguage(dest: Destination, label: string): boolean | null {
    if (!LANGUAGE_CRITERIA.has(label)) return null;
    const langs = (dest.languages ?? []).map(l => normalizeText(l));
    return langs.includes(normalizeText(label));
}
...
function buildDirectCell(level: TableCellLevel, text: string): TableCell {
    return { level, text };
}

function buildFallbackCellFromText(analysis: string, label: string): TableCell {
...
function buildCellForCriterion(
    ...
    if (typeMatch !== null) {
        return buildDirectCell(
            typeMatch ? "yes" : "no",
            typeMatch
                ? "Correspond au type d'échange."
                : "Ne correspond pas au type d'échange.",
        );
    }
...
function buildComparisonText(
    ...
    if (yesCount === cells.length) {
        ...
        const winnerName =
            winnerIndex >= 0
                ? destinations[winnerIndex].shortName ||
                  destinations[winnerIndex].name
                : "Une destination";
...
export function computeDestinationScore(
    tableRows: TableRow[],
